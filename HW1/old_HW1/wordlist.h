...
public:
    ...
    void print_forward(void); // print list from head to end. (Waring if list is empty)
    void print_backward(void); // print list from end to head. (Warning if list is empty)
...
int WordList::insert(const string in, const string pos) // function to insert word 'in' after word 'pos'
{
    ...
    while (curr)
    {
        if (curr->word == pos && curr->pnext == NULL) // if we want to insert a word after the last word in the list
        {
            WordNode* temp = new WordNode; // first, insert a 'space' right after word 'pos'
            temp->word = " ";
            temp->pprev = curr;
            temp->pnext = NULL;
            curr->pnext = temp;
            curr = curr->pnext; // curr node becomes this new 'space'

            temp = new WordNode; // then, insert the word 'in'
            temp->word = in;
            temp->pprev = curr;
            temp->pnext = NULL;
            curr->pnext = temp;
...
// else, we delete the word by traversing the list
    ...
    while (curr != end)
    {
        if (curr->word == st)
        {
            ...
            if (curr->word == " ") // then, if after this word is a space, we delete this space (we don't want to delete a return)
            {
                temp = curr;
                curr->pprev->pnext = curr->pnext;
                curr->pnext->pprev = curr->pprev;
            ...
            else // else, if after this word is a return, we will delete the node before this word, no matter it is a space or a return
            {
                curr = curr->pprev;
                temp = curr;
                curr->pprev->pnext = curr->pnext;
    ...
    while (end->word == st)
    {
        delete end->pnext;// if the last node is the word we want to delete, we first delete the node which saves this word
        WordNode *temp = end;
        ...
        delete end->pnext; // then we delete the space or character return before this word
        temp = end;
        end->pprev->pnext = NULL;
        end = end->pprev;
        delete temp;
...
int WordList::output_backward(const string fileName) // function to output list into document 'filename'
{
    ...
    curr = end;
    if (curr == NULL) { // if end is NULL, do nothing and go to check head.
        curr = head;
    }
    while (curr)
    {
