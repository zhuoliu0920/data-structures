...
public:
    ...
    void check(const FreqList&, string*, int); // function to check the appearance of words in a list(document) from a string array(dictionary)
...
void FreqList::check(const FreqList &F, string *DIC, int dic_size)
{
    FreqNode* index = F.head;

    while (index != NULL) {

        if (bsearch(DIC, 0, dic_size-1, index->word) == -1) {
            FreqList::insert(index->word, index->freq);
        }
        index = index->pnext;
    }
