...
{ 
    ...
    for (int i = 0; i < inputSize-1; i++)
    {
        ...
        else if (!isalnum(data[i+1]))
            temp2.push_back(data[i]);
            // if the data we scan is seperator, and the following character is also seperator, then we store them in temp2 string.

        else if (isalnum(data[i+1])) // if the data we scan is seperator, and next character is a letter, then if there is some word in temp1, insert temp1 to list; if there is some separator in temp2, insert temp2 next to temp1 in the list.
        {
            temp2.push_back(data[i]);
            if (!temp1.empty()) {
                insert(temp1);
    ...
    else // if the last character is a seperator, we push it into temp2 string, and insert into the list. 
    {
        insert(temp1);
